...
// Get env vars
const {
  RECURRENT_REPO_QUERIES_TOPIC_ARN,
} = process.env;
...
export const handler = async (event, context) => {
  ...
  const repoQueries = repos.map(repo => sns.publish({
    ...
    TopicArn: RECURRENT_REPO_QUERIES_TOPIC_ARN,
  }).promise());
  requestLogger.debug({ repoQueryCount: repoQueries.length });

  const sendMessagesResponse = await Promise.all(repoQueries);
  requestLogger.debug({ sendMessagesResponse });
}
